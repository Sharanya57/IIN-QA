...
public class SeleniumFunctions {
	...
	public void CheckElementPresentAndClick(String elmtName) throws Exception {

		WebDriver driver = BaseVariables.getInstance().getWebdriver();
		List<WebElement> ele = driver.findElements(By.xpath(""));
		if (ele.size() == 0) {
			System.out.println("No popup displays");
		} else {
			// close popup
	...
	public String GetTextElement_ShadowDomModulesPage() {
		WebDriver driver = BaseVariables.getInstance().getWebdriver();

		JavascriptExecutor js = (JavascriptExecutor) driver;
		WebElement label = (WebElement) js.executeScript(
				"return document.querySelector('d2l-sequence-viewer').shadowRoot.querySelector('#view-container> #viewframe > d2l-sequences-content-router').shadowRoot.querySelector('d2l-sequences-content-module').shadowRoot.querySelector('div>h1>d2l-sequences-module-name')");
...
/*	public void check() throws InterruptedException {
	...
	public void toggelebuttonvalidation(String pageName, String attributeName, String locatorName, String eletoClick)
			throws Exception {
		String classattr_ = "";
		int i = 0;
	...
	public void CompleteButtonClick() {
		...
		try {
			...
			String imagesFilePath = System.getProperty("user.dir") + "\\" + "skuli" + "\\" + "ss" + "\\";
			Screen s = new Screen();
			Pattern completebox = new Pattern(imagesFilePath + "complete.PNG");

			Thread.sleep(5000);

			// Click on Browse button and handle windows pop up using Sikuli

			s.click(completebox);// come till image path
	...
	public void NextButtonClick() {
		...
		try {
			...
			String imagesFilePath = System.getProperty("user.dir") + "\\" + "skuli" + "\\" + "ss" + "\\";
			Screen s = new Screen();
			Pattern NextBox = new Pattern(imagesFilePath + "Nex.PNG");

			Thread.sleep(2000);

			// Click on Browse button and handle windows pop up using Sikuli

			s.click(NextBox);// come till image path
	...
	public void CalendarDateSelect(String birthDate, String pagename) throws Exception {
		...
		/*
		 * WebElement WebElementmonth = getElement(monthWebElement); WebElement
		 * WebElementyear = getElement(yearWebElement); WebElement WebElementdate =
		 * getElement(dateWebElement);
		 */
		// 22/JAN/2001
		...
		String locvalmonths = BaseVariables.getInstance().GetPageList().get(pagename).getLocatorList()
				.get("GetAllMonthsName").getLocatorValue();
		...
		// GetAllDates
		List<WebElement> lists_dates = Page(pagename).FindElements(LocatorType.XPath, BaseVariables.getInstance()
				.GetPageList().get(pagename).getLocatorList().get("AllDates").getLocatorValue());
	...
	public void selectCountryCodeFromDropDown(String codetoselect, String PageName, String elementName)
			throws Exception {
	...
	public void CheckElementPresent(String elmtName) throws InterruptedException {
		 ...
		 * 
		 * 
		 * wait.until(ExpectedConditions.(By.xpath("//*[text()='"+elmtName+"']"))).
		 * sendKeys(Keys.ARROW_DOWN);
		 * 
		 * 
		 * 
		 * LOGGER.info("Scroll and click  operation"); // waitForPageToLoad();
	...
	public void SliderHdandle(String left, String right, int leftbarage, int rightbarage,String leftelement, String rightageelement) {
		try {
			LOGGER.info("Slider operation");
			WebDriver driver = BaseVariables.getInstance().getWebdriver();

			WebElement left_icon = getElement(left);
			WebElement right_icon = getElement(right);
			WebElement righr_age_text = getElement(rightageelement);
			WebElement left_age_text = getElement(leftelement);
			int lefticonwidth = left_icon.getSize().width;
			int righticonwidth = right_icon.getSize().width;

			while (!(left_age_text.getText().equals(String.valueOf(leftbarage))))
			
			{
				Actions a1 = new Actions(driver);
				a1.dragAndDropBy(left_icon, (lefticonwidth/3) * 1, 0).build().perform();
			 
			}
			while (!(righr_age_text.getText().equals(String.valueOf(rightbarage)))) {

				Actions a2 = new Actions(driver);
				a2.dragAndDropBy(right_icon, -(righticonwidth / 3) * 1, 0).build().perform();
	...
	public void clickbyactionclass(String elementName) {
		try {
			WebElement E2;
			E2 = getElement(elementName);
			System.out.println(E2);
			Actions action = new Actions(driver);
			action.moveToElement(E2).doubleClick().build().perform();
		} catch (Exception e) {
			AssertUtils.getInstance().Fail(e.getMessage());
			LOGGER.info(e.getMessage());
		}
	...
	public Boolean isElementPresent(String elmtName) {
		try {
//PageName="StudentCoachingCircle";
	...
	public void checkImagePresence(String elementName, String imagename) throws Exception {
		 ...
		 * , ImageFile); if (!ImagePresent) { Assert.fail("Image not displayed."); }
	...
	public void validateProfilePage(String[] data, String elmtName) throws InterruptedException {
		...
		// gender,educationlevel,occupation,email, (countrycode+mobilephone)
